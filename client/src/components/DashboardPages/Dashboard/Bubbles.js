...
export default function Bubbles({status, currentPage}) {
    ...
    if(bubbles !== null){
       bubsToRen = bubbles.map(bubble => {
           return <li key={bubble.Sentiment_ID}><Link style={{"textDecoration": "none"}} to= {`/sentiment/${bubble.Sentiment_ID}`}>><button style={{ textDecoration: 'none' }} type ="button">{bubble.Topic_Name}</button></Link>
       </li>
       })
   }
